...
// __attribute__((target("avx"))) int conv8(float *data, const Kernel& kernel, float *res) {
//     // 加载 data 和 kernel 到 SIMD 寄存器
//     __m256 data_vec = _mm256_loadu_ps(data);
//     // 元素乘法
//     __m256 mul = _mm256_mul_ps(data_vec, kernel.get_kernel_vec256()[0]);
//     // 求和
...
// // 水平求和函数
// inline float hsum16_ps(__m512 v) {
...
// // AVX512 优化的卷积函数
// __attribute__((target("avx512f"))) int conv16(float *data, const Kernel &kernel, float *res) {
//     // 加载 data 和 kernel 到 SIMD 寄存器
//     __m512 data_vec = _mm512_loadu_ps(data);
//     __m512 kernel_vec = kernel.get_kernel_vec512()[0];
