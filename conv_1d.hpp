...
public:
    ...
    Conv1D(const float* filter, int window_size) : kernel_(filter, window_size){
        switch (window_size) {
            case 8:
                conv_func_ = [this](float *data, float *res) { this->conv_8(data, res); };
                break;
            default:
...
public:
    ...
    // 卷积函数 avx512版本
    int conv_8(float *data, float *res) __attribute__((target("avx512f")));
